...
class RegistrationValidatorTest(TestCase):
    ...
    def test_required_fields(self) -> None:
        ...
        assert set(validator.errors) == {"username", "name", "password"}

    def test_invalid_email(self) -> None:
        validator = RegistrationValidator(
            data={"username": "not-an-email", "name": "New User", "password": "a-secure-password"}
        )
    ...
    def test_rejects_fields_exceeding_form_limits(self) -> None:
        ...
        assert not validator.is_valid()
        assert set(validator.errors) == {"username", "name"}
    ...
    def test_disallowed_email_domain(self) -> None:
        ...
        assert not validator.is_valid()
        assert validator.errors["username"] == ["Enter a valid email address."]
    ...
    def test_password_uses_email_for_similarity_validation(self) -> None:
        ...
        assert not validator.is_valid()
        assert validator.errors["password"] == ["The password is too similar to the username."]
...
class AuthRegisterEndpointTest(APITestCase):
    ...
    def test_registers_and_authenticates_user(
        ...
        assert parse_qs(next_uri.query)["frontend_events"] == ['{"event_name":"Sign Up"}']
    ...
    def test_duplicate_created_during_registration_returns_validation_error(self) -> None:
        with (
            patch.object(
                RegistrationValidator, "validate_username", return_value="new@example.com"
            ),
