...
def _create_user(data: RegistrationRequest) -> User:
    ...
    try:
        with transaction.atomic(database):
            user = User(username=username, email=username, name=data["name"])
        ...
        if User.objects.filter(username__iexact=username).exists():
            raise serializers.ValidationError({"username": [EMAIL_ALREADY_REGISTERED]}) from None
        raise
...
def _get_registration_success_payload(
    request: Request, user: User
) -> AuthSuccessSerializerResponse:
    """Build the authenticated response while preserving legacy signup navigation."""
    payload = get_auth_success_payload(request, user)
    payload["nextUri"] = add_params_to_url(
        payload["nextUri"],
        {"frontend_events": json.dumps({"event_name": "Sign Up"})},
    )
    return payload
...
class AuthRegisterEndpoint(Endpoint):
    ...
    def post(self, request: Request) -> Response:
        ...
        return Response(_get_registration_success_payload(request, user))
