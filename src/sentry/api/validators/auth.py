...
class RegistrationValidator(CamelSnakeSerializer[RegistrationRequest]):
    username = serializers.EmailField(max_length=128)
    ...
    def validate(self, data: RegistrationRequest) -> RegistrationRequest:
        try:
            password_validation.validate_password(
                data["password"], user=User(username=data["username"])
            )
